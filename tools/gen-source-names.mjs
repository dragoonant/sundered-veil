// gen-source-names.mjs — builds data/names-source.js: the published game's card, deck
// and trait names and its printed rules text, keyed by our internal ids, registered as a
// pack the player can switch on and off (names.js registerSource; default on).
//
// The output is COMMITTED and served by GitHub Pages; it is loaded by index.html only,
// never by tests.html, so the text tests keep exercising the generated describers.
// When the file is absent the game shows the original Sundered Veil names throughout.
// This generator is the ONLY path by which third-party names enter the repo: card data,
// engine, tests and art prompts stay in original terms (CLAUDE.md, NOTICE.md).
//
// Usage:
...
const body = [
  ...
  '  SB.names.registerSource({ cards: C, traits: T, decks: D, text: X });',
].join('\n');

const OUT = join(root, 'data', 'names-source.js');
writeFileSync(OUT,
  '// names-source.js — GENERATED by tools/gen-source-names.mjs; do not hand-edit.\n' +
  '// The published game\'s card, trait and deck names and printed rules text, keyed by\n' +
  '// our internal ids. Loaded by index.html only (never tests.html) and registered as a\n' +
  '// switchable pack: names.js keeps the original Sundered Veil names alongside. Card\n' +
  '// data, engine, tests and art prompts never carry these names; see NOTICE.md.\n' +
  "(function (SB) {\n  'use strict';\n" + body + '\n})(window.SB = window.SB || {});\n');
...
// loads it — via tests.html, which deliberately excludes data/names-source.js, so
// SB.cardText here is the describer output and not the pack we just wrote.
