...
(function (SB) {
  ...
  SB.names = {
    ...
    ui: {
      ...
      // Shown on the title screen (js/title.js) and kept in step with NOTICE.md.
      disclaimer: 'Unofficial, non-commercial fan project. Not affiliated with, endorsed by, or connected to Fantasy Flight Games, Lucasfilm Ltd., or The Walt Disney Company.',
  ...
  // ---- the source-name pack ------------------------------------------------
  // data/names-source.js (generated by tools/gen-source-names.mjs, loaded by index.html
  // only) registers the published game's card, trait and deck names plus its printed
  // rules text, keyed by the same internal ids. Both name sets stay in memory and the
  // player picks one; 'source' is the default when a pack is present. The original
  // theme is kept as the fallback for every id the pack does not cover, and as the
  ...
  function readMode() {
    ...
    mode = (saved === 'original') ? 'original' : 'source';
    return mode;
  }
  function apply() {
    if (!pack) return;
    ...
    // js/text.js: SB.cardText returns these lines verbatim when set, generated text otherwise.
    SB.sourceText = useSource ? (pack.text || null) : null;
  }
  ...
  SB.names.clearSource = function () {
    if (!pack) return;
    mode = mode || readMode();
    const m = mode; mode = 'original'; apply(); mode = m;   // restore every displaced entry
  ...
  SB.names.toggleMode = function () { SB.names.setMode(readMode() === 'source' ? 'original' : 'source'); };
})(window.SB = window.SB || {});
