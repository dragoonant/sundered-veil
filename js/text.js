...
(function (SB) {
  ...
  const opText = {
    ...
    searchDeck: function (op) {
      ...
      const verb = op.playIt ? (', reveal ' + it + ', and play ' + it + (op.playDiscount ? ' for ' + op.playDiscount + ' less' : ''))
        : ', reveal ' + it + ', and draw ' + it;
      let s = 'search ' + (op.depth ? 'the top ' + op.depth + ' cards of ' : '') + 'your deck for ' + filterNoun(op.filter, n) + verb;
    ...
    dividedDamage: function (op) {
      return 'deal ' + amountText(op) + ' damage divided as you choose among ' + scopeNounPlural(op.scope || { who: 'enemy', what: 'unit' });
    },
    ...
    playFromHand: function (op) {
      ...
      if (op.filter && op.filter.requiresPenalty) s += ' that you would owe an aspect penalty for';
      const perks = [];
    ...
    grantDiscount: function (op) {
      return 'each of the next ' + op.count + ' ' + filterNoun(op.filter).replace(/^an? /, '') + 's you play this phase costs ' + op.amount + ' less';
    },
    ...
    giveAdvantage: function (op) {
      if (op.amountRef != null) return 'give an advantage token to ' + targetText(op) + ' ' + amountText(op).replace(/^1 /, '');
      const n = op.amount || 1;
  ...
  function describeAbility(ab) {
    ...
    if (ab.trigger === 'defenderAura') {
      return 'While this unit is defending, the attacker gets ' + statPair((ab.grant || {}).attackerPower, 0) + '.';
    }
  ...
  function attachRule(card) {
    ...
    if (!f) return null;
    if (f.notTrait) return 'Attach to a non-' + traitName(f.notTrait) + ' unit.';
    if (f.trait) return 'Attach to ' + an(traitName(f.trait) + ' unit') + '.';
    if (f.uniqueOnly) return 'Attach to a champion unit.';
    return null;
  }
  ...
  function costRules(card) {
    ...
    if (a && a.delta < 0) {
      const who = (a.cards || []).map(function (id) { return SB.names.card(id); }).join(' or ');
